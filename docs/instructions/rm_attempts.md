...
Why use it?
Lowest fatigue option that still feels like real heavy lifting.
Can be used more often (weekly/biweekly) without wrecking you for sparring.
Keeps you used to handling heavy loads and builds confidence.
...
You almost never lift heavy enough for an RPE 8–9 single to be meaningful.
...
## 3. Estimating 1RM via HS attempts + RPE points
A. Program-Generator Rationale: Heavy Single (HS) + Estimated 1RM
1. Purpose of the HS method in the app
The heavy single (HS) @ submax RPE (usually 8–9) is your low-disruption strength monitoring tool, especially valuable for fighters:
Tracks strength trends without max testing.
Feels “real” to the athlete (a genuinely heavy single).
Minimal extra fatigue if programmed well.
Easy to do relatively often (weekly/biweekly) compared to full RM tests.
The app then uses your 2.5% per RPE step rule to back-calculate a “practical 1RM” from that heavy single.

2. When the generator should use HS + estimated 1RM
Context / phase
Can be used:
In off-season strength blocks and
In fight camp, especially in the middle weeks when you don’t want RM tests.
It’s the primary monitoring tool in camp when true 1RM and 2–5RM tests are suppressed.
Frequency
Typically:
1× per week or 1× every 2 weeks per key lift (e.g., main squat/hinge or press).
The generator should not schedule HS tests for all main lifts in the same week for a fatigued athlete; stagger across weeks if needed.
Lift selection
Only on main barbell / loaded lifts with stable technique:
Squat / front squat / trap bar deadlift
Bench/bench variant
Possibly weighted chin for some fighters
Not on accessories or unstable variants.

3. How the app defines the HS test inside a session
The generator prescribes something like:
Normal warm-up sets (submax).
Then:
“Work up to a heavy single @RPE 8–9 (not a max).”
After logging:
Load used
RPE (8 or 9; you can allow 7–9 if you want more flexibility)
Followed by:
Back-off sets at a lower % (e.g., 3–5×3 at 75–85% of estimated 1RM).
So the HS is embedded as the top set, not a separate “test day”.

4. How the app estimates 1RM from the HS using the 2.5% rule
Assumption from your system:
1 RPE point = ~2.5% of 1RM load (for singles and low reps).
For a single:
RPE 10 ≈ 100% of 1RM
RPE 9 ≈ ~97.5% of 1RM
RPE 8 ≈ ~95% of 1RM
So, if an athlete hits a heavy single at a known RPE, the app can reverse it.
Formula (for singles)
Let:
L_HS = load used for the heavy single
RPE_HS = RPE of that single
Define % of 1RM for RPE:
RPE 10 → 1.00
RPE 9 → 0.975
RPE 8 → 0.95
Then:

est 1RM = L_HS / RPE_multiplier

Where RPE_multiplier is 0.95 or 0.975 depending on RPE.
Examples
150 kg × 1 @ RPE 8
→ est 1RM ≈ 150 / 0.95 ≈ 158 kg
150 kg × 1 @ RPE 9
→ est 1RM ≈ 150 / 0.975 ≈ 154 kg
To keep it simple in code, you can hard-code a small lookup table for HS singles.

5. How the app uses the HS-derived 1RM
Step 1 – Compute est. 1RM from the HS
As above.
Step 2 – Constrain volatility
To avoid wild jumps when RPE is misjudged or athlete has an unusually good/bad day, the generator should:
Compare HS-derived est 1RM to the current training max.
Limit updates, e.g.:
If difference is ≤ ±2.5% → accept change.
If difference is 2.5–7.5% → clamp to ±5%.
If difference >7.5% → treat as outlier and either:
Ignore this single for updating TM, or
Apply a max ±7.5% adjustment.
Step 3 – Update training max
New training max = blended value, if you want more stability:
e.g. 50% old TM, 50% new estimate
(or a rolling average of last 2–3 HS tests)
Or simply:
New TM = old TM + (clamped change).
Step 4 – Recalculate future %s
Future workouts that use %s will use this updated training max.
Day-to-day, the athlete still adjusts actual bar load via RPE (your 2.5% per RPE rule).

6. Why this method is especially good for combat athletes (app logic justification)
Low fatigue, high frequency
HS @ RPE 8–9 is much less draining than repeated near-failure sets or true 1RM attempts. Perfect when skill training load is high.
Fast feedback loop
You can track strength week-to-week without dedicated “testing weeks”.
Skill-specific exposure
Athletes regularly feel what a heavy but manageable single is like, improving:
Confidence with big weights
...
If you choose this (or leave it on default):
The app organizes training in 4-week blocks by default:
3 loading weeks + 1 deload week (3:1 format).
A heavy single @RPE 8–9 on a main lift is scheduled:
Once per block, usually in week 3 (about every 3rd week).
Pattern inside the block:
Week 1–2: Normal progressive work, no formal heavy-single test (just submax top sets).
Week 3: Includes a heavy single @RPE 8–9 as the top set for that lift. The app uses this to update your estimated 1RM.
Week 4: Deload – no heavy single, overall volume and intensity reduced.
In fight camp:
The same 3:1 structure can be used, but
Heavy singles are removed or softened in the final 1–2 weeks before a fight.
Summary to show user:
“With this option, the app uses 3 loading weeks and 1 deload week. You’ll see a heavy but non-max single on a main lift about every 3rd week (in week 3 of each block), and none during deloads or the final taper into a fight.”

2. 2–5RM Test (Estimate 1RM from a Hard Set)
...
Heavy singles and/or 2–5RM tests (if you enabled them) to keep estimates updated.
Summary to show user:
...
By default, the app uses heavy singles in a 3:1 format – three loading weeks and one deload week. You’ll hit a heavy single about every 3rd week, then deload.
